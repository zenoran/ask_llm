...
ASK_LLM_SCHEDULER_ENABLED=true

# How often to check for due jobs (seconds)
ASK_LLM_SCHEDULER_CHECK_INTERVAL_SECONDS=30

# Profile maintenance interval (minutes)
ASK_LLM_PROFILE_MAINTENANCE_INTERVAL_MINUTES=60

# Model to use for profile maintenance (empty = auto-select local model)
ASK_LLM_PROFILE_MAINTENANCE_MODEL=
```
...
psql -d ask_llm -c "SELECT * FROM job_runs ORDER BY started_at DESC LIMIT 10;"
