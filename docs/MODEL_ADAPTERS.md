...
```python
from ask_llm.adapters import get_adapter
...
The adapter is initialized in `BaseAskLLM.__init__()` and stored as `self.adapter`. It's passed to `query_with_tools()` for use in the tool loop.
