...
**NEW FILE: src/ask_llm/adapters/\_\_init\_\_.py**
...
__all__ = ["ModelAdapter", "DefaultAdapter", "get_adapter", "register_adapter"]
```
...
class BaseAskLLM(ABC):
    def __init__(self, ...):
        ...
        # Attach adapter to client for use in streaming
        self.client.adapter = self.adapter
```
...
Also update the call site in base.py `query()` method to pass the adapter.
...
| base.py | MODIFY | ~8 (add adapter init) |
| streaming.py | MODIFY | ~15 (add adapter param, combine stops) |
| react.py | MODIFY | ~-20 (remove hardcoded model quirks) |
