...
ask_llm supports tool calling through a dual-mode architecture: **native tool calling** for providers that support it (OpenAI) and **ReAct format** as a universal fallback for all other models (GGUF, Ollama). A legacy XML format is retained for backward compatibility.
