...
Multiple ask_llm bot personalities can have dedicated Nextcloud Talk rooms. A single webhook endpoint routes messages to the appropriate bot based on conversation token. Room and bot provisioning is handled by an external provisioner service.
...
### Bot Config (`~/.config/ask-llm/bots.yaml`)
...
Service integration: `src/ask_llm/service/api.py` (webhook and admin endpoints)
...
- Check `ASK_LLM_TALK_PROVISIONER_TOKEN` is set and correct
