...
ask_llm supports web search through multiple providers via a pluggable search client architecture. Search results are used as context for LLM responses when the user's query benefits from current information.
...
2. `ASK_LLM_SEARCH_PROVIDER` config setting
...
ASK_LLM_SEARCH_PROVIDER=brave

# Tavily
ASK_LLM_TAVILY_API_KEY=your-key-here

# Brave Search
ASK_LLM_BRAVE_API_KEY=your-key-here
ASK_LLM_BRAVE_SAFESEARCH=moderate    # off, moderate, strict

# General search settings
ASK_LLM_SEARCH_MAX_RESULTS=5
ASK_LLM_SEARCH_TIMEOUT=10
```
