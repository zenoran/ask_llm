...
- **LLM Clients**: `src/ask_llm/clients/` - Abstract `LLMClient` base with implementations for OpenAI, Ollama, GGUF (llama-cpp), HuggingFace
- **Bot System**: `src/ask_llm/bots.py` + `bots.yaml` - Personality-based AI with isolated memory per bot
- **Memory Backend**: `src/ask_llm/memory/postgresql.py` - PostgreSQL + pgvector for semantic search
- **Memory Extraction**: `src/ask_llm/memory/extraction/` - LLM-based fact extraction from conversations
- **User Profiles**: `src/ask_llm/user_profile.py` - SQLModel ORM for user preferences injected into system prompts
- **Background Service**: `src/ask_llm/service/` - Optional FastAPI service for async/background LLM queries

### Data Flow
```
CLI args → Config (pydantic-settings) → AskLLM → LLMClient → Response
                                          ↓
...
1. Create `src/ask_llm/clients/your_client.py` extending `LLMClient` from `base.py`
...
### Bot Configuration
Bots are defined in `src/ask_llm/bots.yaml`. Each bot has:
...
### Memory System
- **Entry point plugin**: `pyproject.toml` registers `ask_llm.memory` via `discover_memory_backends()` in `core.py`
...
### Configuration
Config uses `pydantic-settings` with `ASK_LLM_` env prefix. Key files:
- `~/.config/ask-llm/.env` - API keys and database credentials
...
- Run with: `llm-service` or `ask-llm-service`
- Client: `ServiceClient` in `src/ask_llm/service/client.py`
...
pipx runpip ask-llm install ruff mypy pytest pytest-cov pytest-mock
```
...
### Debug memory issues
- Check `POSTGRES_*` env vars in `~/.config/ask-llm/.env`
...
1. Add `Field()` to `Config` class in `src/ask_llm/utils/config.py`
2. Use `ASK_LLM_` prefix for environment variable
3. Access via `config.YOUR_SETTING` in components

### Add a new bot
1. Add bot definition to `src/ask_llm/bots.yaml`
...
pipx runpip ask-llm install <package>
```
