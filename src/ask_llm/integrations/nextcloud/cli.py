...
def provision(ask_llm_bot, room_name, bot_name, owner):
    ...
    if manager.get_bot(ask_llm_bot):
        console.print(f"[red]Error:[/red] Bot '{ask_llm_bot}' already has Nextcloud config")
        console.print(f"Remove it first with: [cyan]llm nextcloud remove --bot {ask_llm_bot}[/cyan]")
        return

    # Defaults
    if not room_name:
        room_name = ask_llm_bot.title()
    if not bot_name:
        bot_name = ask_llm_bot.title()

    console.print(f"[cyan]Provisioning Nextcloud Talk room...[/cyan]")
    console.print(f"  Bot: {ask_llm_bot}")
...
@click.option('--bot', 'ask_llm_bot', required=True, help='ask_llm bot ID to remove Nextcloud config from')
def remove_bot(ask_llm_bot):
    """Remove Nextcloud Talk configuration from a bot."""
    manager = get_nextcloud_manager()

    if not manager.get_bot(ask_llm_bot):
        console.print(f"[red]Error:[/red] Bot '{ask_llm_bot}' has no Nextcloud config")
        return

    if click.confirm(f"Remove Nextcloud config from bot '{ask_llm_bot}'?"):
        manager.remove_bot(ask_llm_bot)
        console.print(f"[green]✓[/green] Removed Nextcloud config from bot: {ask_llm_bot}")


@nextcloud_cli.command(name='rename')
@click.option('--bot', 'ask_llm_bot', required=True, help='ask_llm bot ID')
@click.option('--name', 'new_name', required=True, help='New room name')
def rename_room(ask_llm_bot, new_name):
    """Rename a Nextcloud Talk room."""
    manager = get_nextcloud_manager()

    bot_config = manager.get_bot(ask_llm_bot)
    if not bot_config:
        console.print(f"[red]Error:[/red] Bot '{ask_llm_bot}' has no Nextcloud config")
        return
