    ...
    if message and msg_type == "Create":
        try:
            ...
            request_obj = ChatCompletionRequest(
                ...
                user=user_id,
                bot_id=bot_config.ask_llm_bot,  # Use the bot from config!
                stream=False,
            )
            ...
            log.error(f"Error processing message with bot {bot_config.ask_llm_bot}: {e}", exc_info=True)
