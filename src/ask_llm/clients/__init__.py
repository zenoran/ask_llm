from .base import LLMClient
from .openai_client import OpenAIClient
from .ollama_client import OllamaClient

try:
    import torch
    import transformers
    from .huggingface_client import HuggingFaceClient
    _hf_available = True
except ImportError:
    # Fallback stub if HuggingFace dependencies are missing
    class HuggingFaceClient(LLMClient):  # type: ignore
        """Stub client when huggingface dependencies are not installed."""
        def __init__(self, *args, **kwargs):
            raise ImportError("HuggingFaceClient dependencies not installed")
    _hf_available = False

try:
    from llama_cpp import Llama # Check if base library is installed
    from .llama_cpp_client import LlamaCppClient
    _llama_cpp_available = True
except ImportError:
    LlamaCppClient = None # type: ignore # Set to None if unavailable
    _llama_cpp_available = False

__all__ = ['LLMClient', 'OpenAIClient', 'OllamaClient']
if _hf_available and HuggingFaceClient:
    __all__.append('HuggingFaceClient')
if _llama_cpp_available and LlamaCppClient:
    __all__.append('LlamaCppClient')

del _hf_available
del _llama_cpp_available

